...
| Method | Endpoint | Notes |
|---|---|---|
| GET | `/api/notifications` | Own notifications, newest first (max 100) |
| PATCH | `/api/notifications/:id/read` | Mark one as read |

### Alerts (Phase 7)
| Method | Endpoint | Role | Notes |
|---|---|---|---|
| GET | `/api/alerts` | Patient or Caregiver | Patient sees their own; caregiver sees all actively-linked patients' alerts |
| PATCH | `/api/alerts/:id/resolve` | Caregiver | Marks an alert resolved |

**How alerts get created:** `alerts/services.py::check_and_create_alert()` runs
automatically every time a dose is marked missed (hooked into
`POST /api/doses/:id/missed`). It reuses the same threshold from
`analytics.THRESHOLDS['high_missed_count']` (3 missed doses in 7 days by
default) that powers the Phase 6 risk indicator — so the two stay
consistent. It won't spam duplicate alerts; it updates the existing
unresolved one instead. Every actively-linked caregiver gets a
`Notification` the moment an alert fires.

**Smart reminders (upcoming/overdue dose nudges):** there's no task queue
in this project (deliberately, to avoid a Celery dependency for a project
this size), so reminders are generated by a management command instead:

```bash
python manage.py generate_reminders
```

Run it periodically (every 10–15 minutes) via cron (Linux/Render) or
Windows Task Scheduler. It's idempotent — running it often is safe, it
